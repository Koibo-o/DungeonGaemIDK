            ...
            while (play) {
                ...
                while (monster.getHealth() > 0 && player.getH() > 0) {
                    ...
                    switch (characterType) { // Special Move Handler
                        case "Light":
                            damageDealt = handleLightMoves(pm, monster, player, vm);
                            if (pm.equals("dodge")) {
                                reduceDamage = true;
                            } else if (pm.equals("poison")) {
                                poisonEffect = true;
                            }
                            break;
                        case "Normal":
                            damageDealt = handleNormalMoves(pm, monster, player, monsterDamage, vm);
                            if (pm.equals("parry")) {
                                reduceDamage = true;
                            }
                            if (pm.equals("boost")) {
                            ...
                            break;
                        case "Heavy":
                            damageDealt = handleHeavyMoves(pm, monster, player, vm);
                            if (pm.equals("tank")) {
                                reduceDamage = true;}
                            if(pm.equals("challenge")){
                                challi = true;}
                            break;
                    }
